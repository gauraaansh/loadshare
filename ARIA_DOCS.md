...
Covered in depth in **Section 5 (ML Models)**. Short version: tabular data + small-to-medium dataset size + SHAP interpretability requirement + operational explainability → XGBoost wins consistently.
...
This section provides dense, direct answers to the most common technical questions about ARIA. Each subsection is a standalone Q&A.
...
4. SHAP feature importances are the primary "why did this rider get an intervention?" explainability signal for operators
...
For this deployment context (single workstation, fixed services, no horizontal scaling requirement) Docker Compose is the correct tool.
...
- **No API cost at demo scale:** Each agent cycle makes 4–6 LLM calls. At GPT-4 pricing, running demos at TIME_SCALE=100 (a full simulated day in ~15 real minutes) would generate hundreds of cycles per hour — cost becomes prohibitive at this scale.
...
- **Free tier sufficient:** at demo scale (one user, one demo session at a time), Cloudflare's free tunnel plan handles the traffic.
...
**vs Neural Networks (transformer-based tabular):** FT-Transformer and TabNet are emerging alternatives that match or exceed XGBoost on large tabular datasets. They require significantly more tuning and have no mature SHAP equivalent. For a system requiring interpretability (SHAP feature importances displayed in the UI), XGBoost is the correct choice.
...
Loadshare's live operational data is not publicly available, and Indian gig-economy rider operations data at ARIA's required granularity does not exist in any public dataset.
